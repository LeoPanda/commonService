...
/**
 * Google　OAuth2 認証のためのパネルを作成するクラス
 * Google OAuth2認証を実装し、googleサービスへ接続するために必要な認証tokenを提供する。
 * @author LeoPanda
 * <pre>
 *使用するには
...
public class GoogleLoginBar extends HorizontalPanel{
	...
	private HostAuthServiceAsync hac_ 
			= GWT.create(HostAuthService.class);	//RPCハンドル
	//メンバ変数
	...
	//メンバ変数のGetter
	public String getToken()	{return loginToken_;}
	public String getGmailAdress() 	{return gmailAdress_;}
	public String getGoogleInnerId() 	{return googleInnerId_;}
	public String getGoogleUserName() { return googleUserName_;}
	public String getGoogleUserIconUrl(){ return googleIconUrl_;}
	//Googleサービスのスコープ	
	...
	public void addListerner(LoginBarListener listener){listener_ = listener;} //リスナーのセッター
	//認証結果通知イベントオブジェクト
	...
	public GoogleLoginBar(String clientId,final String apiScope){
		...
		this.add(signInLink);
		//google ログイン状態のチェック
		hac_.loginCheck(GWT.getHostPageBaseURL(),
	...
	/**
	 * Google認証tokenの取得を行う。
	 * 認証取得手続きは非同期のため、取得後の処理はイベントハンドラの応答を待ってから行われる。
	...
	private void getGoogleToken(String moreScope) {
		...
		final AuthRequest req = 
				new AuthRequest(googleAuthUrl,GOOGLE_CLIENT_ID).withScopes(apiScope); 
		//認証トークンの取得
	...
	/**
	 * google パーソナル情報の取得
	 * getGoogleToken()から呼び出される。
	 * @param token 	Google認証token
	 */

...
public class HostAuthServiceImpl extends RemoteServiceServlet 
		implements HostAuthService{
	private static final long serialVersionUID = 1L;
	/**
	 *Google OAuth Loginチェック
	 *UserService.Userから取得出来る情報をLoginInfoに返す
	 *<pre>
	 * @param String requestUr : OAuth2 request Uri
	 ...
	 *Google User情報の取得
	 *<pre>
	 * @param String token : OAhth2 認証token
	 * @param LoginInfo ログイン情報 
	 * </pre>
	 */
	public LoginInfo loginDetails(final String token) {
		...
		try {
			...
			String jsonSource = urlService.fetchGet(urlStr, urlService.addToken(token, 
									urlService.setHeader(ContentType.XML))).get(Result.BODY);
